...
func start(scriptPath string) {
	analysis, err := execShellCheck(scriptPath)
	if err != nil {
		log.Fatal(err)
	}
	...
	if writeFile {
		if err := os.WriteFile(scriptPath, []byte(result), 0600); err != nil {
			log.Fatalf("could not write updated script to file: %v", err)
		}

		printf("\n%s %s\n", color.GreenString("Updated script written to"), color.GreenString(scriptPath))
	} else {
		printf("\n%s\n\n", color.GreenString(result))
	}

	printf("%s\n", color.YellowString("CAUTION: This script was generated by an LLM and "+
		"may contain errors or nonsensical content. Please review thoroughly before committing."))
}

func execShellCheck(scriptPath string) (string, error) {
	cmd := exec.Command("shellcheck", scriptPath)
